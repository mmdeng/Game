...
namespace Tetris
{
	public class Draw
	{
		...
		public static void MakeBackGround(Data data)
		{
			...
			// 背景塗りつぶし
			var rectangle = data.MAIN_AREA.GetRectangle();
		...
		private static void DrawScore(Data data, Graphics g, Brush brush, Font font)
		{
			...
			var message = "Score: " + data.score.TotalScore.ToString();
		...
		private static void DrawFieldBlock(Data data, Graphics g)
		{
			for (var x = 0; x < data.X_MAX; x++)
			{
				for (var y = 0; y < data.Y_MAX; y++)
				{
					...
					var source = GetBitmapRectangle(data, data.FIELD_BLOCK[x, y]);
		...
		private static void DrawFallingBlock(Data data, Graphics g)
		{
			...
			for (var x = 0; x < data.CurrentBlock.Width; x++)
			{
				for (var y = 0; y < data.CurrentBlock.Height; y++)
				{
					...
					var source = GetBitmapRectangle(data, data.CurrentBlock[x, y]);
					g.DrawImage(data.BLOCK_BITMAP, target, source, GraphicsUnit.Pixel);
				}
			}
		}
		...
		{
			for (var x = 0; x < data.NextBlock.Width; x++)
			{
				for (var y = 0; y < data.NextBlock.Height; y++)
				{
					...
					var source = GetBitmapRectangle(data, data.NextBlock[x, y]);
					g.DrawImage(data.BLOCK_BITMAP, target, source, GraphicsUnit.Pixel);
				}
			}
		}
		...
		/// <returns></returns>
		private static Rectangle GetBitmapRectangle(Data data, int type)
		{
